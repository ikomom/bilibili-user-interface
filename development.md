...
Frontend, built with Docker, with routes handled based on the path: <http://localhost:8173>

Backend, JSON based web API based on OpenAPI: <http://localhost:8888>
...
For the backend and frontend, they use the same port that would be used by their local development server, so, the backend is at `http://localhost:8888` and the frontend at `http://localhost:8173`.
...
PostgreSQL: <postgresql://localhost:8432>
